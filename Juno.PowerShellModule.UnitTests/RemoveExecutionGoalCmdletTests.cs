namespace Juno.PowerShellModule
{
    ...
    public class RemoveExecutionGoalCmdletTests
    {
        private IAsyncPolicy retryPolicy;
        private int currentRetries;
        private TestRemoveExecutionGoalCmdlet cmdlet;
        ...
        public void SetupTest()
        {
            ...
            this.cmdlet = new TestRemoveExecutionGoalCmdlet(this.retryPolicy);
            this.cmdlet.TeamName = this.expectedTeamName;
            this.cmdlet.ExecutionGoalId = this.expectedId;
        ...
        public void CmdletThrowsExpectedExceptionWhenClientThrowException()
        {
            ...
            this.mockDependencies.ExperimentClient.Setup(m => m.DeleteExecutionGoalAsync(It.IsAny<string>(), It.IsAny<string>(), It.IsAny<CancellationToken>()))
                .Callback((string executionGoalId, string teamName, CancellationToken token) =>
                {
                    Assert.AreEqual(this.cmdlet.ExecutionGoalId, executionGoalId);
            ...
            Assert.AreEqual(this.expectedId, this.cmdlet.ExecutionGoalId);
            Assert.IsFalse(this.cmdlet.AsJson);
            Assert.IsFalse(this.cmdlet.IsJsonObject);
            Assert.IsNull(this.cmdlet.Results);
        ...
        private class TestRemoveExecutionGoalCmdlet : RemoveExecutionGoalCmdlet
        {
            public TestRemoveExecutionGoalCmdlet(IAsyncPolicy retryPolicy)
                : base(retryPolicy)
            {
            }
