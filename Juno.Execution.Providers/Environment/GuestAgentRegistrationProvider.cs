namespace Juno.Execution.Providers.Environment
{
    ...
    /// Guest Agent (and its agent ID) with the experiment. By registering the agent with the experiment,
    /// this enables operations that run in the process of the Guest Agent to determine with which experiment
    ...
    public class GuestAgentRegistrationProvider : ExperimentProvider
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="GuestAgentRegistrationProvider"/>
        /// </summary>
        /// <param name="services">The set of services/dependencies for the provider.</param>
        public GuestAgentRegistrationProvider(IServiceCollection services)
            : base(services)
        {
        }
