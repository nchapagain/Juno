namespace Juno.EnvironmentSelection.NodeSelectionFilters
{
    ...
    public class SsdModelFilterProviderTests
    {
        ...
        public void SetupTests()
        {
            ...
            this.provider = new SsdModelFilterProvider(this.mockFixture.Services, this.mockFixture.Configuration, NullLogger.Instance);
        }
        ...
        public async Task SsdFilterProviderSearchesCorrectNodesWithBothDriveTypes()
        {
            using (IMemoryCache<IEnumerable<string>> cache = new MemoryCache<IEnumerable<string>>())
            {
                ...
                IEnumerable<string> includeModel = this.mockFixture.SelectionSettings.SsdModels;
                string excludeModel = includeModel.Shuffle().First();
                IEnumerable<string> driveTypes = new string[] { SsdDriveType.Data.ToString(), SsdDriveType.System.ToString() };

                EnvironmentFilter filter = new EnvironmentFilter(typeof(SsdModelFilterProvider).FullName, new Dictionary<string, IConvertible>()
                {
                    { "includeCluster", string.Join(",", clusterIncludeList) },
                    { "excludeCluster", clusterExcludeList },
                    { "includeModel", string.Join(",", includeModel) },
                ...
                Assert.IsTrue(candidates.Values.All(candidate => candidate.AdditionalInfo[ProviderConstants.SsdModel].ToList(',').All(sf => includeModel.Contains(sf))));
                Assert.False(candidates.Values.All(candidate => candidate.AdditionalInfo[ProviderConstants.SsdModel].ToList(',').Any(sf => sf.Equals(excludeModel, StringComparison.OrdinalIgnoreCase))));
