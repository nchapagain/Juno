namespace Juno.EnvironmentSelection.NodeSelectionFilters
{
    ...
    public class BioSFilterProviderTests
    {
        ...
        public void SetupTests()
        {
            ...
            this.provider = new BiosFilterProvider(this.mockFixture.Services, this.mockFixture.Configuration, NullLogger.Instance);
        }

        [Test]
        public async Task BiosFilterProviderSearchesCorrectNodes()
        {
            using (IMemoryCache<IEnumerable<string>> cache = new MemoryCache<IEnumerable<string>>())
            {
                ...
                IEnumerable<string> includeBios = this.mockFixture.SelectionSettings.BiosVersions;
                string excludeBios = includeBios.Shuffle().First();
                ...
                EnvironmentFilter filter = new EnvironmentFilter(typeof(BiosFilterProvider).FullName, new Dictionary<string, IConvertible>()
                {
                    { "includeCluster", string.Join(",", includeCluster) },
                    { "excludeCluster", excludeCluster },
                    { "includeBiosVersion", string.Join(",", includeBios) },
                    { "excludeBiosVersion", excludeBios }
                });
                ...
                // Validate only correct Bios Version were returned
                Assert.IsFalse(candidates.Values.Any(candidate => candidate.AdditionalInfo["BIOSVersion"].Equals(excludeBios)));
                Assert.IsTrue(candidates.Values.All(candidate => includeBios.Contains(candidate.AdditionalInfo["BIOSVersion"])));
