namespace Juno.EnvironmentSelection.NodeSelectionFilters
{
    ...
    public class BlockedNodesFilterProviderTests
    {
        ...
        public void SetupTests()
        {
            ...
            this.provider = new BlockedNodesFilterProvider(this.mockFixture.Services, this.mockFixture.Configuration, NullLogger.Instance);
        }

        [Test]
        public async Task BlockedNodesFilterProviderSearchesCorrectNodes()
        {
            using (IMemoryCache<IEnumerable<string>> cache = new MemoryCache<IEnumerable<string>>())
            {
                ...
                EnvironmentFilter filter = new EnvironmentFilter(typeof(BlockedNodesFilterProvider).FullName, new Dictionary<string, IConvertible>()
                {
