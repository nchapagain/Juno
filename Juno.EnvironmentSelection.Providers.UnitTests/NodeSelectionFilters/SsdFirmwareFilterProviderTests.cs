namespace Juno.EnvironmentSelection.NodeSelectionFilters
{
    ...
    {
        private TestSsdFirmwareFilter provider;
        ...
        public void SetupTests()
        {
            ...
            this.provider = new TestSsdFirmwareFilter(this.fixture.Services, this.fixture.Configuration, this.fixture.Logger.Object);
        ...
        public void PopulateCacheAsyncValidatesParameters()
        {
            ...
            EnvironmentFilter filter = new EnvironmentFilter(typeof(TestSsdFirmwareFilter).FullName);
        ...
        public void PopulateCacheAsyncPostsExpectedCacheKeys()
        {
            ...
            {
                { new ProviderCacheKey() { { ProviderConstants.SsdFirmware, "fw1,fw2,fw3,fw4" } },  expectedValues }
            };

            IDictionary<ProviderCacheKey, IList<string>> expectedKeys = new Dictionary<ProviderCacheKey, IList<string>>()
            {
                { new ProviderCacheKey() { { ProviderConstants.SsdFirmware, "fw1" } }, expectedValues },
                { new ProviderCacheKey() { { ProviderConstants.SsdFirmware, "fw2" } }, expectedValues },
                { new ProviderCacheKey() { { ProviderConstants.SsdFirmware, "fw3" } }, expectedValues },
                { new ProviderCacheKey() { { ProviderConstants.SsdFirmware, "fw4" } }, expectedValues },
            };
        ...
        {
            ProviderCacheKey expectedKey = new ProviderCacheKey() { { ProviderConstants.SsdFirmware, "fw1" } };
            string expectedNodeId = Guid.NewGuid().ToString();
            EnvironmentCandidate expectedCandidate = new EnvironmentCandidate(node: expectedNodeId, additionalInfo: new Dictionary<string, string> { [ProviderConstants.SsdFirmware] = Guid.NewGuid().ToString() });
        ...
        public void RetrieveCacheHitsAsyncReturnsExpectedResultWhenMultipleKeysReferenceSameNode()
        {
            ...
            string expectedNodeId = Guid.NewGuid().ToString();
            List<string> expectedFirmwares = new List<string>() { Guid.NewGuid().ToString(), Guid.NewGuid().ToString() };

            EnvironmentCandidate expectedCandidate = new EnvironmentCandidate(node: expectedNodeId, additionalInfo: new Dictionary<string, string> { [ProviderConstants.SsdFirmware] = expectedFirmwares[0] });
            EnvironmentCandidate expectedCandidate2 = new EnvironmentCandidate(node: expectedNodeId, additionalInfo: new Dictionary<string, string> { [ProviderConstants.SsdFirmware] = expectedFirmwares[1] });
            ...
            Assert.AreEqual(1, actualResult.Count);
            Assert.IsTrue(actualResult.ContainsKey(expectedNodeId));
            Assert.AreEqual(2, actualResult[expectedNodeId].AdditionalInfo[ProviderConstants.SsdFirmware].ToList(',').Count);
            Assert.IsTrue(actualResult[expectedNodeId].AdditionalInfo[ProviderConstants.SsdFirmware].ToList(',').All(sku => expectedFirmwares.Contains(sku)));
        }
        ...
        private class TestSsdFirmwareFilter : SsdFirmwareFilterProvider
        {
            public TestSsdFirmwareFilter(IServiceCollection collection, IConfiguration configuration, ILogger logger)
