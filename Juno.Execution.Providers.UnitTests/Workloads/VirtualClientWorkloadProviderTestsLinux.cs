namespace Juno.Execution.Providers.Workloads
{
    ...
    [TestFixture]
    [Category("Unit")]
    public class VirtualClientWorkloadProviderTestsLinux
    {
        ...
        [Test]
        public async Task VirtualClientWorkloadProviderValidatesRequiredParameters()
        {
            this.mockFixture.Component.Parameters.Clear();
            Dictionary<string, IConvertible> requiredParameters = new Dictionary<string, IConvertible>
            {
        ...
        [Test]
        public async Task VirtualClientWorkloadProviderCreatesTheExpectedProcessToRunTheLinuxVirtualClientWorkloadWithParametersSpecifiedForArm64()
        {
        ...
        public async Task VirtualClientWorkloadProviderWritesASpecificationFileWhenRequiredDataIsAvailable()
        {
            ...
            this.mockVmEntities.First().Metadata["dataDisks"] = VmDisk.ToString(VirtualClientWorkloadProviderTestsLinux.CreateDataDisks());
        ...
        [Test]
        public async Task VirtualClientWorkloadProviderDoesNotWriteASpecificationFileUnlessItIsRequestedAsPartOfTheExperimentStepDefinition()
        {
            // Move to step where VC is started.
            this.providerState.DependenciesInstalled = true;

            // The 'includeSpecifications' parameter is NOT defined in the parameters
            await this.provider.ExecuteAsync(this.mockFixture.Context, this.mockFixture.Component, CancellationToken.None);
        ...
        public void VirtualClientWorkloadProviderWritesTheSpecificationFileToTheExpectedLocation()
        {
            ...
            this.mockFixture.DataClient.OnGetEntitiesProvisioned().ReturnsAsync(this.mockVmEntities);
            string command = VirtualClientWorkloadProviderTestsLinux.CreateVirtualClientExePath(this.mockFixture.Component);
        ...
        public async Task VirtualClientWorkloadProviderWritesTheExpectedInformationToTheSpecificationFile()
        {
            ...
            // OS and Data disk SKU information is defined on the VM entities when they are registered
            // with the experiment.
            IEnumerable<VmDisk> expectedDataDisks = VirtualClientWorkloadProviderTestsLinux.CreateDataDisks();
            this.mockVmEntities.First().Metadata["osDiskSku"] = "Standard_LRS";
        ...
        public async Task VirtualClientWorkloadProviderCanBeCancelledBeforeRunningTheVirtualClient()
        {
            ...
            {
                VirtualClientWorkloadProviderTestsLinux.RequestCancellation(tokenSource);
        ...
        public async Task VirtualClientWorkloadProviderReturnsTheExpectedResultWhenTheVirtualClientProcessExit()
        {
            ...
            this.mockProcess.Setup(process => process.Id)
              .Returns(VirtualClientWorkloadProviderTestsLinux.currentProcessId++);
        ...
        private static string CreateVirtualClientExePath(ExperimentComponent component)
        {
            var packageVersion = component.Parameters.GetValue<string>(StepParameters.PackageVersion);
            if (component.Parameters.TryGetValue(StepParameters.Platform, out IConvertible platform) && ((string)platform == VmPlatform.LinuxX64 || (string)platform == VmPlatform.LinuxArm64))
            {
                string command = $"{{NuGetPackagePath}}/virtualclient/{packageVersion}/content/{platform}/VirtualClient";
                component.Parameters.Add(StepParameters.Command, command);
                return command;
            }
            else
            {
                string command = $"{{NuGetPackagePath}}\\VirtualClient\\{packageVersion}\\content\\win-x64\\VirtualClient.exe";
                component.Parameters.Add(StepParameters.Command, command);
                return command;
            }
        }
