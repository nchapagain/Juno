...
REM This operation uses a custom target defined in the Juno.CRCBladeCertificationAgent.csproj file that defines the specific OS flavors to 
REM build and additionally creates a NuGet package containing each of the flavors in separate directories.
Echo [Building Juno CRC Blade Certification Agent]
echo --------------------------------------------------
call dotnet publish "%~dp0Juno.CRCBladeCertificationAgent\Juno.CRCBladeCertificationAgent.csproj" -r win-x64 -c Debug && echo: || Goto :Error
Goto :Finish
...
:Finish
echo Juno CRC Blade Certification Agent Build Exit Code: %ExitCode%
exit /B %ExitCode%